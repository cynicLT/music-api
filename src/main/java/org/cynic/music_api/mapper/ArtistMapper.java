...
public abstract class ArtistMapper {
    ...
    public Optional<ArtistHttp> to(ObjectNode item) {
        return Optional.ofNullable(internal(
            convert(item, ItunesJsonFieldPointer.ARTIST_REF_ID.fieldName()),
            convert(item, ItunesJsonFieldPointer.ARTIST_NAME.fieldName())
        ));
    }

    protected Optional<JsonNode> convert(ObjectNode item, String path) {
